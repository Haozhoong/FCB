...
class FNormUnet(nn.Module):
    ...
    def __init__(
        ...
        out_chans: int = 2,
        drop_prob: float = 0.0,
        num_rows: int = 320, num_cols: int = 320,
        fcb: int = 1, forward: bool = False
    ):
        """
        Args:
        ...
        self.unet = FUNet(
            ...
            fcb=fcb, forward=forward
        )
    def repara(self, model_load):
        self.unet.repara(model_load.unet)
...
class FVarNet(nn.Module):
    ...
    def __init__(
        ...
        self.cascades = nn.ModuleList(
            [VarNetBlock(FNormUnet(chans=chans,
                                   num_rows=num_rows, num_cols=num_cols,
                                   fcb=fcb, forward=forward)) for _ in range(num_cascades)]
        )
    ...
    def repara(self, model_load):
        ...
        self.sens_net.norm_unet.repara(model_load.sens_net.norm_unet)
        for cascade, cascade_load in zip(self.cascades, model_load.cascades):
            cascade.model.unet.repara(cascade_load.model.unet)
